...
public class Forest implements Serializable {
            ...
            switch (dungeon.uzivatVolba("Cut trees", "Hunt birds", "Open inventory and info", "Go in town")) {
                case 1:
                    ...
                    if (dungeon.getForAll().energy < 10) {
                        ...
                        continue NABIDKA;
                    } else if (loadCapacityControl(dungeon.getForAll().woodHeft, foundWood, dungeon) > 0) {
                        foundWood = loadCapacityControl(dungeon.getForAll().woodHeft, foundWood, dungeon);
                ...
                case 2:
                    if (dungeon.getForAll().energy < 20) {
                        ...
                        if (dungeon.getRand().nextInt(100) < dungeon.getForAll().birdChance) {
                            ...
                            if (loadCapacityControl(dungeon.getForAll().birdHeft, foundBirds, dungeon) > 0) {
                                foundBirds = loadCapacityControl(dungeon.getForAll().birdHeft, foundBirds, dungeon);
    ...
    private int loadCapacityControl(int heft, int count, Dungeon dungeon) {
        for (int i = count; i > 0; i--) {
            if (heft * i + dungeon.getForAll().lCapacity > dungeon
