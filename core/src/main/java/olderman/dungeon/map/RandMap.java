...
public class RandMap implements Serializable {
    ...
    public void mapBarrier() {
        mapPosition = data.w * 3 + (data.rightEdge * 2 + 1) * data.w + data.l * 2;
    ...
    public void asciiArtMap() {
        ...
        if (eastRoom == null) {
            mapPosition = data.w * 3 + (data.rightEdge * 2 + 1) * data.w + wayEast * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);
        }
        if (northRoom == null) {
            mapPosition = wayNorth * 3 + (data.rightEdge * 2 + 1) * wayNorth + data.l * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);

        }
        if (southRoom == null) {
            mapPosition = waySouth * 3 + (data.rightEdge * 2 + 1) * waySouth + data.l * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);

        }
        if (westRoom == null) {
            mapPosition = data.w * 3 + (data.rightEdge * 2 + 1) * data.w + wayWest * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);
    ...
    public void mapInvisibility() {
        ...
        if (eastRoom == null) {
            mapPosition = dungeon.getWay().previousW * 3 + (data.rightEdge * 2 + 1) * dungeon.getWay().previousW + wayEast * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);
        }
        if (northRoom == null) {
            mapPosition = wayNorth * 3 + (data.rightEdge * 2 + 1) * wayNorth + dungeon.getWay().previousL * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);

        }
        if (southRoom == null) {
            mapPosition = waySouth * 3 + (data.rightEdge * 2 + 1) * waySouth + dungeon.getWay().previousL * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);

        }
        if (westRoom == null) {
            mapPosition = dungeon.getWay().previousW * 3 + (data.rightEdge * 2 + 1) * dungeon.getWay().previousW + wayWest * 2;
            data.map = changeCharInPosition(mapPosition, MapConstants.wallChar, data.map);

        }
        mapPosition = dungeon.getWay().previousW * 3 + (data.rightEdge * 2 + 1) * dungeon.getWay().previousW + dungeon.getWay().previousL * 2;
        if (dungeon.getWay().previousL == dungeon.bossY && dungeon.getWay().previousW == dungeon.bossX) {
